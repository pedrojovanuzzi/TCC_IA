...
def login(data: TokenRequest):
    ...
    return {
        "access_token": criar_token({"user_id": r[0], "nivel": r[1], "username": r[2]}),
        "token_type": "bearer"
    }
...
def listar(token=Depends(verificar_token)):
    ...
    conn = get_connection(); c = conn.cursor()
    c.execute("SELECT id, login, nivel, name FROM users")
    u = [{"id": i[0], "login": i[1], "nivel": i[2], "name": i[3]} for i in c.fetchall()]
    conn.close()
    return u
...
def criar(user: dict = Body(...), token=Depends(verificar_token)):
    if not user.get("login") or not user.get("password"):
        raise HTTPException(400, "Dados obrigatórios")
    if not isinstance(user.get("nivel", 1), int) or not 1 <= user["nivel"] <= 3:
    ...
    return {"success": True, "login": user["login"], "nivel": user["nivel"], "name": name}
...
def atualizar(user_id: int, data: dict = Body(...), token=Depends(verificar_token)):
    ...
    if data.get("login"):
        u.append("login=%s"); p.append(data["login"])
    if data.get("password"):
        h = hashlib.sha256(data["password"].encode()).hexdigest()
        u.append("password=%s"); p.append(h)
    if data.get("nivel") is not None:
        u.append("nivel=%s"); p.append(data["nivel"])
    if data.get("name"):
        u.append("name=%s"); p.append(data["name"])

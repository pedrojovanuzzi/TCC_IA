...
from app.config import CORES_CLASSES, ENCRYPTION_KEY, IMG_REAL_TIME_DIR, IMG_SIZE, MODEL_PATH, MONITORINGFRAMES
...
async def webcam_client():
    ...
    async with websockets.connect(uri, max_size=None) as ws:
        # URL da câmera RTSP
        rtsp_url = MONITORINGFRAMES
        cap = cv2.VideoCapture(rtsp_url)

        if not cap.isOpened():
            print("❌ Erro ao acessar a câmera RTSP.")
            return

        print("🎥 Enviando frames da câmera RTSP para o servidor...")
        while True:
            ret, frame = cap.read()
            if not ret:

...
const regex = /^(?:(\d+\s*D))?(?:\s*(\d+\s*H))?(?:\s*(\d+\s*M))?(?:\s*(\d+\s*S))?$/i;
  ...
  // ⚠️ Validação do formato
  if (!regex.test(tempo.trim()) || tempo.trim() === "") {
    setErro(
      "Formato inválido. Use a ordem: Dias → Horas → Minutos → Segundos (ex: '1D 2H 30M')."
    );
    return;
  }
  ...
  return (
    <><Header></Header><div className="flex justify-center  mt-10 ">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          <div className="flex flex-col">
            ...
            <input
              ...
              className={`border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 ${erro
                  ? "border-red-500 focus:ring-red-500"
                  : "focus:ring-blue-500"}`} />
            {erro && (
              <p className="text-sm text-red-600 mt-1 font-medium">{erro}</p>
            )}
          </div>
          ...
          <button
            ...
            className={`${erro
                ...
                : "bg-gradient-to-r from-blue-600 to-blue-400 hover:from-blue-700 hover:to-blue-500"} text-white font-semibold py-2 rounded-lg shadow-md transition-all duration-300`}
          >
            Salvar Configuração
          </button>
        </form>
